...
!
! Longitud y latitud (grados), y altura (metros) de los observatorios
! con el formato DD.MMSS
! Por convenio longitudes son positivas hacia el Este y negativas hacia
! el Oeste.
!
      IMPLICIT NONE
      INCLUDE 'proas.inc'
...
!
      DATA OBSER/'CALAR ALTO','EL TEIDE','MADRID','LA PALMA',&
              &'SAN PEDRO','LICK','LA SILLA','PARANAL','MAUNA KEA',&
              &'McDonald','KPNO','LLANO DEL HATO','CERRO TOLOLO'/
!
      WRITE(*,104)
      WRITE(*,101)'ENTER OBSERVATORY'
      WRITE(*,*)
      WRITE(*,160)0,'Observatory data through keyboard &
...
! San Pedro (datos aproximados extraidos de un mapa mundi)
        LAT=32.0
        LONG=-115
        ALTOBS=2000.
      ELSE IF(NOPC.EQ.6)THEN
! Lick (datos facilitados por Jesus Gallego)
        LAT=37.2036
        LONG=-123.3812
        ALTOBS=1283.
